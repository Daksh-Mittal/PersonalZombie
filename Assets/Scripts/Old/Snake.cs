    ...
    private void OnTriggerEnter2D(Collider2D collider)
{
    ...
    else if (State == SnakeState.Attack && collider.gameObject == Frog)
    {
    ...
    void FixedUpdateEvents()
    {
        ...
        {
            if (Vector2.Distance(transform.position, Frog.transform.position) >= DeAggroRange)
            {
    ...
    void FSM_State()
    {
        ...
        {
            desiredVel = Steering.Flee(transform.position, _target, MaxSpeed, AvoidParams);
        }
    ...
    private void UpdateAppearance()
    {
        ...
        {
            _sr.color = new Color(0.0f, 0.0f, 0.25f);
        }
